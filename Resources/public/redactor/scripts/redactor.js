...
//            var eId     = this.$el.get(0).id;
//            var element = document.getElementById(eId);
//            var val     = innerXHTML(element, html);
		...
		{
            var etags = ["<pre></pre>","<blockquote>\\s*</blockquote>","<acronym>\\s*</acronym>","<abbr.*>\\s*</abbr>","<dfn>\\s*</dfn>","<em>\\s*</em>","<ul></ul>","<ol></ol>","<li></li>","<table></table>","<tr></tr>","<span>\\s*<span>", "<span>&nbsp;<span>", "<b>\\s*</b>", "<b>&nbsp;</b>", "<p>\\s*</p>", "<p>&nbsp;</p>",  "<p>\\s*<br />\\s*</p>", "<div>\\s*</div>", "<div>\\s*<br />\\s*</div>"];
		...
		showLink: function()
		{
			...
			// in IE preallocation of a link in modal window does won't work
			// so this section was modified
			var text = '', url = '', target = '';
			
			// modified 
			var text   = this.getSelection();
			var fix    = this.getSelectedHtml();
			...
			// selection of a tag fixed when included in another tag 
			if ($(fix).attr('href')) {
				url = $(fix).attr('href');
			}
			else if ($(fix).find('a').attr('href')) {
				url = $(fix).find('a').attr('href');
			}
			
			if ($(fix).attr('target')) {
				url = $(fix).attr('target');
			}
			else if ($(fix).find('a').attr('target')) {
				target = $(fix).find('a').attr('target');
			}
		...
		insertLink: function()
		{
		    ...
		    // double click selection of a phrase in IE always includes the empty space at the end and integrates it into the tag
			if ($.browser.msie) {
				str = '<a href="' + link + '"' + target + '>' +  $.trim(text) + '</a>';
			}
			else {
				str = '<a href="' + link + '"' + target + '>' +  $.trim(text) + '</a>';
			}
				
			this._insertLink(str, $.trim(text), link, target);
